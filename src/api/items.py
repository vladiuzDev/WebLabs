...
def put_item(item_id: UUID, payload: ItemPut, db: Session = Depends(get_db)) -> ItemRead:
    service = ItemService(db)
...
def patch_item(item_id: UUID, payload: ItemPatch, db: Session = Depends(get_db)) -> ItemRead:
    service = ItemService(db)
